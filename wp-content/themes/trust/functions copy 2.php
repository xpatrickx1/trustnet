...
if ( strpos($url, 'blog') ) {
    add_filter( 'aioseop_description', 'aioseo_filter_description' );
    add_filter( 'aioseop_title', 'aioseo_filter_title' );
}

function aioseo_filter_description( $description ) {
    $paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
    if ( $paged !== 1 && is_category() ) {
        $description = "Go through our blog and enrich your knowledge of the academic field. Well-researched articles and competent insights based on professional experience - Page " . $paged;
    } else if( $paged === 1 && is_category() ) {
        $description = "Go through our blog and enrich your knowledge of the academic field. Well-researched articles and competent insights based on professional experience.";
    }
    return $description;
}
function aioseo_filter_title( $title ) {
    $paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
    if ( $paged !== 1 && is_category() ) {
        $title = "Nursingpaper Blog | Nursing Paper - Page " . $paged;
    } else if( $paged === 1 && is_category() ) {
        $title = "Nursingpaper Blog | Nursing Paper";
    }
    return $title;
}
